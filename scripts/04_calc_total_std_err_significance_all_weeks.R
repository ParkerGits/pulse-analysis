...
generate_total_se_for_weeks <- function(df, week_nums) {
  all_diff_ses_total <- list()
  
  for (week_num in week_nums) {
    print(str_glue("Week {week_num}"))
    
    # create directory if it doesn't exist
    dir.create("data/intermediate-data/week-total-ses", showWarnings = F)
    se_total_week_filepath <- str_glue("data/intermediate-data/week-total-ses/se_total_{week_num}.csv")
    
    # if already computed, read it from file and continue
    if(file.exists(se_total_week_filepath)) {
      se_total_week <- read_csv(se_total_week_filepath)
      all_diff_ses_total <- all_diff_ses_total |>
        append(list(as.data.table(se_total_week)))
    ...
    # generate all_week_list
    all_week_list <- puf_cur_week |>
      ...
      janitor::clean_names() |>
      split(puf_cur_week$week_num)
    
    # computes ses
    start <- Sys.time()
    plan(multisession, workers = parallel::detectCores() - 2)
    all_diff_ses_total_week <- map_dfr(all_week_list, 
                                  ~generate_se_state_and_cbsas_total(metrics = metrics,
                                                                     df = .x))
    end <- Sys.time()
    print(end - start)
    
    all_diff_ses_total_week_dt <- all_diff_ses_total_week |>
      as.data.table()
    
    # write to file
    write_csv(all_diff_ses_total_week_dt, se_total_week_filepath)
    
    # store in list
    all_diff_ses_total <- all_diff_ses_total |> 
      append(list(all_diff_ses_total_week_dt))
  }
  
  return(rbindlist(all_diff_ses_total))
}

puf_all_weeks <- read_csv("data/intermediate-data/puf_formatted.csv")
total_ses <- generate_total_se_for_weeks(puf_all_weeks, 13:67)

write_csv(total_ses, "data/intermediate-data/phase2_all_total_ses.csv")
